...
# --- 1. 앱의 기본 설정 ---
st.set_page_config(page_title="실험실 재고 관리기 v36", layout="wide")
st.title("🔬 실험실 재고 관리기 v36")
...
def load_reagent_db(_client):
    try:
        ...
        # (3. 중복 Lot 합산: agg()에 ASCII 키워드 사용)
        df_agg = df.groupby(['제품명', 'Cat. No.', 'Lot 번호'], as_index=False).agg(
            agg_qty=('최초 수량', 'sum'),       # (ASCII 키워드 'agg_qty')
            agg_unit=('단위', 'last'),          
...
with tab3:
    ...
    if df_db.empty:
        ...
        # (v35/v27 방식: data_editor + column_config)
        st.data_editor( 
